...
// --- CONFIGURACIÓN COMERCIAL REAL ---
const WHATSAPP_NUMBER = "5493512046802"; // Tu número actualizado
const TRANSFER_DISCOUNT = 0.10; // 10% OFF
...
const ProductDetail = ({ product, onClose }) => {
  ...
  const sendWhatsApp = () => {
    const text = encodeURIComponent(`¡Hola Ikigai! Me interesa la figura ${product.name} de ${product.anime} ($${product.price.toLocaleString('es-AR')}). ¿Cómo podemos coordinar el envío?`);
  ...
  const startCheckout = async () => {
    setPaying(true);
    try {
      // Llamamos al backend para generar el link de pago real
      const API_URL = window.location.hostname === 'localhost' ? 'http://localhost:5000/api/create-preference' : '/api/create-preference';
      ...
      alert("Hubo un problema al conectar con Mercado Pago. Intenta por WhatsApp.");
    } finally {
      setPaying(false);
    }
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] flex items-center justify-center p-4 md:p-10">
      ...
      <motion.div initial={{ scale: 0.9, y: 50, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.9, y: 50, opacity: 0 }} className="relative bg-[#0a0a0a] border border-white/10 w-full max-w-6xl rounded-[3rem] overflow-hidden flex flex-col md:flex-row max-h-[90vh] md:overflow-hidden">
        ...
        <div className="w-full md:w-1/2 p-8 md:p-16 flex flex-col justify-center bg-[#0a0a0a]">
          <span className="text-cyan-500 text-xs font-black uppercase tracking-[0.5em] mb-4">{product.anime}</span>
          ...
          <div className="space-y-6">
            <p className="text-[10px] text-gray-500 font-black uppercase tracking-[0.3em]">Formas de Pago Disponibles</p>
            <div className="flex gap-8 items-center opacity-60 grayscale hover:grayscale-0 hover:opacity-100 transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
              <button onClick={sendWhatsApp} className="py-6 bg-[#25D366] text-white font-black uppercase tracking-[0.3em] rounded-2xl hover:brightness-110 transition-all flex items-center justify-center gap-3 text-xs shadow-xl shadow-green-500/10">
              ...
              <button 
                ...
                disabled={paying}
                className="py-6 bg-white text-black font-black uppercase tracking-[0.3em] rounded-2xl hover:bg-cyan-500 transition-all text-xs shadow-xl active:scale-95 disabled:opacity-50"
              >
                {paying ? 'Procesando...' : 'Comprar Ahora'}
              </button>
            </div>
          </div>
        </div>
...
const ProductCard = ({ product, index, onClick }) => (
  <motion.div 
    layout initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} transition={{ delay: (index % 4) * 0.05, duration: 0.5 }}
  ...
  >
    <div className="h-[350px] relative overflow-hidden bg-black">
      ...
      <div className="absolute top-6 right-6">
        <span className="bg-black/60 backdrop-blur-md text-cyan-400 text-[9px] font-black px-4 py-2 rounded-full border border-white/10 uppercase tracking-[0.2em]">{product.anime}</span>
      </div>
    </div>
    <div className="p-8">
...
function App() {
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white selection:bg-cyan-500 selection:text-black font-sans overflow-x-hidden">
      <WhatsAppButton />
      <AnimatePresence>{selectedProduct && <ProductDetail product={selectedProduct} onClose={() => setSelectedProduct(null)} />}</AnimatePresence>
      <nav className="fixed w-full z-50 px-6 md:px-10 py-8">
        <div className="max-w-7xl mx-auto flex justify-between items-center bg-black/40 backdrop-blur-3xl px-6 md:px-10 h-24 rounded-[2rem] border border-white/10 shadow-2xl">
          ...
          <div className="hidden lg:flex gap-16 text-[11px] font-black uppercase tracking-[0.5em] text-white/60">
            <a href="#catalog" onClick={(e) => { e.preventDefault(); scrollToCatalog(); }} className="hover:text-cyan-400 transition-all uppercase tracking-widest">Catálogo</a>
            <a href="#" className="hover:text-cyan-400 transition-all uppercase tracking-widest">Formas de Pago</a>
          </div>
          <button className="p-4 bg-white/10 rounded-2xl hover:bg-cyan-500 transition-all border border-white/20">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4H6zM3 6h18M16 10a4 4 0 01-8 0"/></svg>
          </button>
        </div>
      </nav>
      ...
      <section className="relative h-[110vh] flex flex-col justify-center items-center text-center px-10">
        ...
        <motion.div style={{ y: titleY, opacity }} className="z-10 relative">
          ...
          <button onClick={scrollToCatalog} className="px-16 py-8 bg-cyan-500 text-black font-black text-sm rounded-[2rem] tracking-[0.4em] uppercase hover:shadow-[0_0_50px_rgba(6,182,212,0.5)] transition-all active:scale-95">Explorar Catálogo</button>
        </motion.div>
      </section>
      ...
      <section ref={catalogRef} id="catalog" className="max-w-[1400px] mx-auto px-6 md:px-10 py-40">
        ...
        <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          <AnimatePresence mode="popLayout">{filteredProducts.map((product, index) => <ProductCard key={product.id} product={product} index={index} onClick={setSelectedProduct} />)}</AnimatePresence>
        </motion.div>
      </section>
      ...
      <footer className="py-40 border-t border-white/5 px-10 bg-black/50 backdrop-blur-3xl">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-20">
          <div>
            <Logo />
            <p className="mt-8 text-gray-500 max-w-sm font-medium leading-relaxed uppercase text-[10px] tracking-[0.2em]">Tu destino premium para figuras de anime originales. Importaciones directas y calidad certificada.</p>
          </div>
          <div className="flex flex-col items-center md:items-end gap-10">
            <p className="text-[10px] text-gray-500 font-black uppercase tracking-[0.4em]">Pagos Seguros & WhatsApp</p>
            <div className="flex gap-8 grayscale hover:grayscale-0 transition-all opacity-40 hover:opacity-100 items-center">
            ...
            <p className="text-cyan-500 font-bold text-xs tracking-widest">+54 9 351 204-6802</p>
          </div>
        </div>
      </footer>
    </main>
